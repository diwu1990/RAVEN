...
def snapshot(model, folder, epoch, args):
    path = os.path.join(folder, 'model_{}_raven_{}_{}_{}_{}.pth'.format(epoch, args.cycle, args.intwidth_max, args.fracwidth_max, args.bitwidth_reduce))
...
def main():
    ...
    optimizer = optim.Adam(modelRAVEN.parameters(), lr=args.lr, weight_decay=args.weight_decay)

    best_acc = test(test_loader, modelRAVEN, criterion, device)
    for epoch in range(1, args.epochs + 1):
        acc = train(train_loader, modelRAVEN, criterion, optimizer, epoch, device)
