...
import numpy as np
import math
from RAVEN.pe.raven.ops import *
...
class PrimaryCaps(nn.Module):
    ...
    def __init__(self, A=32, B=32, K=1, P=4, stride=1, 
        ...
        self.bitwidth_reduce = bitwidth_reduce
        self.rounding = rounding
        self.sigmoid = RAVENsigmoid(self.cycle, self.intwidth, self.fracwidth, self.bitwidth_reduce, self.rounding)
...
class ConvCaps(nn.Module):
    ...
    def __init__(self, B=32, C=32, K=3, P=4, stride=2, iters=3,
        ...
        self.bitwidth_reduce = bitwidth_reduce
        self.rounding = rounding
        self.div = RAVENdiv(self.cycle, self.intwidth, self.fracwidth, self.bitwidth_reduce, self.rounding)
        self.log = RAVENlog(self.cycle, self.intwidth, self.fracwidth, self.bitwidth_reduce, self.rounding)
        self.sigmoid = RAVENsigmoid(self.cycle, self.intwidth, self.fracwidth, self.bitwidth_reduce, self.rounding)
        self.softmax = RAVENsoftmax(2, self.cycle, self.intwidth, self.fracwidth, self.bitwidth_reduce, self.rounding)
    ...
    def m_step(self, a_in, r, v, eps, b, B, C, psize):
        ...
        """
        r = r * a_in
        r = self.div(r, (r.sum(dim=2, keepdim=True) + eps))
                         
        r_sum = r.sum(dim=1, keepdim=True)
        coeff = self.div(r, (r_sum + eps))
        ...
        cost_h = (self.beta_u.view(C, 1) + self.log(sigma_sq.sqrt())) * r_sum
    ...
    def e_step(self, mu, sigma_sq, a_out, v, eps, b, C):
        """
            ...
            Output:
                r:         (b, B, C, 1)
        """
        ln_p_j_h = self.div(-1. * (v - mu)**2, (2 * sigma_sq)) \
                   - self.log(sigma_sq.sqrt()) \
                   - 0.5*self.ln_2pi
        
        ln_ap = ln_p_j_h.sum(dim=3) + self.log(a_out.view(b, 1, C))
        ...
        ln_ap = (self.div((ln_ap - ln_ap_min), (ln_ap_max - ln_ap_min)) - 0.5) * 5
