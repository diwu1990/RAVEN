...
class PrimaryCaps(nn.Module):
    ...
    def forward(self, x):
        print("PrimaryCaps: self.pose conv", self.pose)
        print("PrimaryCaps: self.pose conv in", x.shape)
        p = self.pose(x)
        print("PrimaryCaps: self.pose conv out", p.shape)
        print()
        print("PrimaryCaps: self.a conv", self.a)
        print("PrimaryCaps: self.a conv in", x.shape)
        a = self.a(x)
        print("PrimaryCaps: self.a conv out", a.shape)
        print()
        a = self.sigmoid(a)
        print("PrimaryCaps: self.sigmoid", a.shape)
        print()
...
class ConvCaps(nn.Module):
    ...
    def m_step(self, a_in, r, v, eps, b, B, C, psize):
        ...
        """
        r = r * a_in
        print("m-step element mul", r.shape)
        print()
        print("m-step div mul", r.shape)
        print("m-step div", (r.sum(dim=2, keepdim=True)).shape)
        print()
        r = r / (r.sum(dim=2, keepdim=True) + eps)
        r_sum = r.sum(dim=1, keepdim=True)
        print("m-step div mul", r.shape)
        print("m-step div", r_sum.shape)
        print()
        coeff = r / (r_sum + eps)
        coeff = coeff.view(b, B, C, 1)
        
        print("m-step element mul", (coeff * v).shape)
        print()
        mu = torch.sum(coeff * v, dim=1, keepdim=True)
        print("m-step element square mul", (v - mu).shape)
        print()
        print("m-step element mul", (coeff * (v - mu)).shape)
        print()
        ...
        print("m-step element mul", (self._lambda*(self.beta_a - cost_h.sum(dim=2))).shape)
        print()
        a_out = self.sigmoid(self._lambda*(self.beta_a - cost_h.sum(dim=2)))
        print("m-step sigmoid", a_out.shape)
        print()
    ...
    def e_step(self, mu, sigma_sq, a_out, v, eps, b, C):
        """
            ...
            Output:
                r:         (b, B, C, 1)
        """
        print("e-step element square mul", (v - mu).shape)
        print()
        print("e-step div", (-1. * (v - mu)**2 / (2 * sigma_sq)).shape)
        print()
        print("e-step log", (sigma_sq).shape)
        print()
        ln_p_j_h = -1. * (v - mu)**2 / (2 * sigma_sq) \
                    - torch.log(sigma_sq.sqrt()) \
                    - 0.5*self.ln_2pi
        print("e-step log", (a_out).shape)
        print()
        ln_ap = ln_p_j_h.sum(dim=3) + torch.log(a_out.view(b, 1, C))
        ln_ap_min, _ = torch.min(ln_ap, dim=2)
        ln_ap_min = torch.unsqueeze(ln_ap_min, 2)
        ln_ap_max, _ = torch.max(ln_ap, dim=2)
        ln_ap_max = torch.unsqueeze(ln_ap_max, 2)
        print("e-step div", (ln_ap).shape)
        print()
        ln_ap = (((ln_ap - ln_ap_min) / (ln_ap_max - ln_ap_min)) - 0.5) * 5
        print("e-step softmax dim 2", (ln_ap).shape)
        print()
    ...
    def transform_view(self, x, w, C, P, w_shared=False):
        ...
        print("transform_view matmul", x.shape, w.shape)
        print()
        v = torch.matmul(x, w)
        v = v.view(b, B, C, P*P)
        return v
