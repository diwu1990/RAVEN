...
class PrimaryCaps(nn.Module):
    ...
    def __init__(self, A=32, B=32, K=1, P=4, stride=1, 
        ...
        self.appr_grad = appr_grad
        self.fxp_grad = fxp_grad
    ...
    def forward(self, x):
        ...
        a = MACsigmoid(a, 
                       ...
                       fracwidth = self.fracwidth, 
                       fxp = self.fxp, 
                       rounding = self.rounding, 
                       keepwidth = self.keepwidth, 
                       appr_grad = self.appr_grad, 
                       fxp_grad = self.fxp_grad)
...
class ConvCaps(nn.Module):
    ...
    def m_step(self, a_in, r, v, eps, b, B, C, psize):
        ...
        """
        r = r * a_in
        r = MACdiv.apply(r, (r.sum(dim=2, keepdim=True) + eps), 
                         self.cycle, 
                         self.intwidth, 
                         self.fracwidth, 
                         self.fxp, 
        ...
        coeff = MACdiv.apply(r, (r_sum + eps), 
                             self.cycle, 
                             self.intwidth, 
                             self.fracwidth, 
                             self.fxp, 
        ...
        cost_h = (self.beta_u.view(C, 1) + MAClog.apply(sigma_sq.sqrt(), 
                                                        self.cycle, 
                                                        self.intwidth, 
                                                        self.fracwidth, 
                                                        self.fxp, 
                                                        ...
                                                        self.appr_grad, 
                                                        self.fxp_grad)) * r_sum
        a_out = MACsigmoid(self._lambda*(self.beta_a - cost_h.sum(dim=2)), 
                           cycle = self.cycle, 
                           intwidth = self.intwidth, 
                           fracwidth = self.fracwidth, 
                           fxp = self.fxp, 
    ...
    def e_step(self, mu, sigma_sq, a_out, v, eps, b, C):
        """
            ...
            Output:
                r:         (b, B, C, 1)
        """
        ln_p_j_h = MACdiv.apply(-1. * (v - mu)**2, (2 * sigma_sq), 
                             self.cycle, 
                             self.intwidth, 
                             self.fracwidth, 
                             self.fxp, 
        ...
        ln_ap = ln_p_j_h.sum(dim=3) + MAClog.apply(a_out.view(b, 1, C), 
                                                   self.cycle, 
                                                   self.intwidth, 
                                                   self.fracwidth, 
                                                   self.fxp, 
        ...
        ln_ap = (MACdiv.apply((ln_ap - ln_ap_min), (ln_ap_max - ln_ap_min), 
                             self.cycle, 
                             self.intwidth, 
                             self.fracwidth, 
                             self.fxp, 
